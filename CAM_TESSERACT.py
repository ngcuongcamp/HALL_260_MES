...
class MyApplication(QMainWindow):
    ...
    def scan_product_code(self):
        ...
        if self.data_scan1 is not None:
            ...
            # get old txt_packing_qty
            old_packing_qty = int(
                get_title_obj(self, self.MES_PACKING_QTY_ID, self.MES_PACKING_QTY_TYPE)
            )
            new_packing_qty = None
            cmd_printer("INFO", f"old packing qty: {old_packing_qty}")
            ...
            while time.time() - stime <= self.MAX_WAIT:
                ...
                new_packing_qty = int(
                    get_title_obj(
                        self, self.MES_PACKING_QTY_ID, self.MES_PACKING_QTY_TYPE
                    )
                )

                if new_packing_qty == old_packing_qty + 1:
                    cmd_printer("INFO", f"new packing qty: {new_packing_qty}")
                    # PASS MES
